...
class MainMenu:
    ...
    def setup_background(self):
        ...
        self.caption = tools.get_image(setup.GRAPHICS['title_screen1'], 1, 60, 176, 88, (255, 0, 220), C.BG_MULTI)
    ...
    def update_cursor(self, keys):
        if keys[pygame.K_UP]:
            ...
            self.cursor.state = 'PLAY    GAME'
            self.cursor.rect.y = 405
        elif keys[pygame.K_RETURN]:
            if self.cursor.state == 'SUPER UTSER':
                self.finished = True
            elif self.cursor.state == 'PLAY    GAME':
                self.finished = True
